...
- **Status:** DONE (local, uncommitted). Follow-up 2026-07-19 — two bugs and one performance fix landed together:
  ...
  - **Ops:** to enable the webhook: Supabase Dashboard → Database → Webhooks → New Hook. Table=`IncomingLead`, Event=`UPDATE`, Type=`HTTP Request`, URL=`https://<bot-host>/webhooks/supabase/lead-assigned`, Header `x-webhook-secret: <value of SUPABASE_LEAD_WEBHOOK_SECRET>`. Env var must be set on the bot host. Poller runs whether the webhook is configured or not — the two paths are safely idempotent.
