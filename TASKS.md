...
- **Status:** DONE (local, uncommitted). Yoram branch fires in `buildContent` (Galit exceptions digest with real leads counts — see D4-T1). Sasha branch fires at top of `runDigestDispatcher` loop when phone matches `SASHA_PHONE` — sends LEADS_MORNING at 09:30 and suppresses normal MORNING/EVENING via `continue`. Residual elevated path (other ADMIN users, not Yoram, not Sasha) now gated by `LEGACY_MANAGER_DIGEST_ENABLED` (X-T5, default OFF) — skipped before dedup claim when flag is unset. All three K3-defined routing paths implemented and tested (`galitManagerDispatcher.test.ts` + `legacyManagerGate.test.ts` + `sashaLeadsDispatcher.test.ts`).
...
- **Status:** DONE (local, uncommitted). K4 option (c) implemented — env-gated behind new `LEGACY_MANAGER_DIGEST_ENABLED` (default OFF), following `LEGACY_DAILY_SUMMARY_ENABLED` precedent. `dispatchOne` in `src/scheduler/jobs/digestDispatcher.ts` extended: for ADMIN rows whose phone matches neither `YORAM_PHONE` nor `SASHA_PHONE`, when flag is unset/false the send is skipped BEFORE `claimDigestSend` (no dedup row written — slot remains available if flag later enabled). Yoram still fires via the phone-matched branch in `buildContent`; Sasha never reaches `dispatchOne` (loop `continue` before it). `dispatchOne` signature extended with `sashaPhoneNormalized` param. `.env.example` documented with reference to K4 and precedent. New test file `src/__tests__/legacyManagerGate.test.ts` — 8 cases: flag unset/false blocks ADMIN, flag true allows ADMIN, Yoram exempt, Sasha short-circuits at loop level, MANAGER role unaffected, YORAM_PHONE unset combinations. Tests `inspectorMorningDispatcher.test.ts` + `equipmentReminderDispatcher.test.ts` + `galitManagerDispatcher.test.ts` updated to set the flag on ADMIN test cases where the legacy path is asserted.
