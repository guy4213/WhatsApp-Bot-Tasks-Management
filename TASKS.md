...
**Files changed:**
- `src/db/migrations/020_microsoft_graph.sql` (חדש) — 3 טבלאות: `MicrosoftAccount`, `MicrosoftGraphSubscription`, `MicrosoftGraphEventLog`. Idempotent, `gen_random_uuid()`, RLS deny-all (סגנון של 019). **תיקון לאחר הרצה ראשונה שנכשלה:** `"userId"` הוגדר `uuid` בגרסה הראשונית, אך `"User"."id"` הוא `text` ב-DB הזה (כמו בכל המיגרציות הקיימות 001/008/009/016). שונה ל-`text`. Migration הורצה בהצלחה — 3 טבלאות + כל האינדקסים + RLS קיימים.
...
- **Status:** DONE (local, uncommitted). Follow-up 2026-07-19 — two bugs and one performance fix landed together:
  - **Bug 1 (filter):** the poller's `findNewlyAssignedLeads` predicate was `ownerId IS NOT NULL` only. In prod, `IncomingLead` rows arrive with an interim `ownerId` already populated by the CRM before the claim actually happens, so the alert would (silently, once the WhatsApp path warmed up) fire before the CRM claim. Filter now also requires `il.status = 'ACTIVE'` — the CRM's real ownership-transition signal.
  - **Bug 2 (race between webhook + poller):** the dedup layer was check-then-act — `isLeadNotificationSent` SELECT → send → `claimLeadNotification` INSERT. Two callers passing the pre-check within milliseconds would both send WhatsApp and only race on the INSERT afterwards. Migration `022_lead_notification_status.sql` adds a `status` column ('PENDING'|'SENT'). `leadNotificationLog.ts` now exports `tryClaimLeadNotification` (INSERT-first with ON CONFLICT ... DO UPDATE stale-reclaim after 5 min), `markLeadNotificationSent` (UPDATE), and `releaseLeadNotificationClaim` (DELETE on send failure). D3-T3 flow is now atomic; D3-T4 escalation flow left on legacy check-then-act because it has only one caller (the poller) and no race.
  - **New (near-instant delivery):** new route `src/routes/supabaseLeadWebhook.ts` (`POST /webhooks/supabase/lead-assigned`, registered in `app.ts`). Supabase Database Webhook on `IncomingLead UPDATE` calls it within ~1 sec of the CRM claim regardless of channel (CRM UI / voice assistant / bot). Auth: timing-safe `x-webhook-secret` against `SUPABASE_LEAD_WEBHOOK_SECRET`, fail-closed → 404. Transition filter: NEW→ACTIVE OR ownerId-just-set-on-ACTIVE (catches the CRM's split-write race between the two columns without a sleep). Poller kept as safety net; cron dropped from `*/2` to `*/15` in `scheduler/index.ts`.
  - **Extracted `processAssignmentAlertForLead(leadId, preloaded?)`** in `leadAssignmentNotifier.ts` — single-lead handler reused by both the poller loop (with preload) and the webhook (without). Returns `'sent' | 'skipped' | 'no-phone' | 'ineligible' | 'failed'`. Never throws.
  - **Tests:** `leadAssignmentNotifier.test.ts` D3-T3 block rewritten for the INSERT-first flow (claim → send → mark; release on failure); D3-T4 unchanged. New `supabaseLeadWebhook.test.ts` — auth/transition/happy-path/uninteresting-UPDATE/missing-id. `incomingLeads.test.ts` `findNewlyAssignedLeads` case updated to assert the new predicates. All 45 tests in these three files pass; `npx tsc --noEmit` clean.
  - **Ops (superseded 2026-07-19b, kept for history):** Supabase Dashboard → Database → Webhooks → New Hook. Table=`IncomingLead`, Event=`UPDATE`, Type=`HTTP Request`, URL=`https://<bot-host>/webhooks/supabase/lead-assigned`, Header `x-webhook-secret: <SUPABASE_LEAD_WEBHOOK_SECRET value>`. **Do NOT create this hook if migration 023 is applied — the DB trigger replaces it. Running both would double every notification.**
  - **Follow-up 2026-07-19c — removed `u.role != 'ADMIN'` filter from `findNewlyAssignedLeads` + `findAssignedLeadById`.** Product feedback (Guy Franses): managers/admins also do field work and are legitimate lead assignees; silently skipping their alert was a bug, not a feature. Any user with a phone that gets a lead assigned now receives the alert regardless of role. Test regression guard added — the assertion in `incomingLeads.test.ts::findNewlyAssignedLeads` now uses `expect(sql).not.toMatch(/u\.role\s*!=\s*'ADMIN'/)` so a future re-introduction of the filter fails CI. TypeScript clean; all 45 tests in the three relevant files still pass.
  - **Follow-up 2026-07-19b — migration 023: replaced UI-managed Dashboard Webhook with a Postgres trigger.** A DB-managed webhook fires on EVERY UPDATE and the bot filters on receive; wasted round trips at scale. Migration `023_lead_assigned_trigger.sql` (a) enables `pg_net`, (b) creates `BotWebhookConfig` (deny-all-public RLS) with `lead_assigned_url` + `lead_assigned_secret` placeholder rows, (c) creates SECURITY DEFINER function `notify_lead_assigned_worker()` that reads the config, builds a Supabase-shape JSON payload, and calls `net.http_post` (async, transactional), (d) creates trigger `trg_lead_assigned_worker AFTER UPDATE ON "IncomingLead" FOR EACH ROW WHEN (NEW.status = 'ACTIVE' AND (OLD.status IS DISTINCT FROM 'ACTIVE' OR (NEW.ownerId IS NOT NULL AND OLD.ownerId IS NULL)))`. The WHEN clause is filtered by Postgres — irrelevant UPDATEs never enqueue an HTTP call. Payload matches Supabase Dashboard Webhook shape, so `POST /webhooks/supabase/lead-assigned` needs no code change. Post-migration operator step: UPDATE the two `BotWebhookConfig` rows with the real URL + secret (SQL block at the bottom of the migration file). Placeholder values (`'%REPLACE-ME%'`) short-circuit the function to a no-op — a half-configured deploy is silent, not broken.
- **Prior status (kept for history):** New file `src/scheduler/jobs/leadAssignmentNotifier.ts` (`runLeadAssignmentNotifier` → `processAssignmentAlerts` + `processEscalations`). D3-T3 path: `findNewlyAssignedLeads()` → INSERT-first `claimLeadNotification(leadId, 'ASSIGNED_TO_WORKER')` → `sendTextMessage` to worker (alert: sender, subject, body, "לטיפול ועדכון ב-CRM"). Skips workers with no phone. Per-lead failures isolated. Registered in `scheduler/index.ts` at `*/2 * * * *`, lock ID 1010. Dedup is via `WhatsappLeadNotification` (migration 010) — NOT via `IncomingLead.notifiedAt` (CRM-owned column not written). Tests: in `src/__tests__/leadAssignmentNotifier.test.ts`.
