...
describe('resolveCalibration — refuses outside the departure window', () => {
  it('returns null when the poll arrives > 20 min after departedAt AND no cache', () => {
    const ratio = resolveCalibration({
      taskFieldId: TF,
      travelEtaMinutes: 55,
      departedAt: new Date(T0.getTime() - 21 * 60 * 1000), // 21 min ago
      currentBaseSeconds: 12 * 60,
      now: T0,
    });
  ...
  it('accepts capture at the 20-min boundary (edge)', () => {
    const ratio = resolveCalibration({
      taskFieldId: TF,
      travelEtaMinutes: 55,
      departedAt: new Date(T0.getTime() - 20 * 60 * 1000),
      currentBaseSeconds: 24 * 60,
      now: T0,
    });
  ...
  it('accepts capture at 15 min post-departure (widened window covers typical customer open-times)', () => {
    // The 5-min window before the 2026-07-09 widening was routinely missed
    // because the customer opened the tracking link a few minutes after
    // receiving the WhatsApp message. Two widenings (5→10→20) later, this
    // case documents the current tolerance.
    const ratio = resolveCalibration({
      taskFieldId: TF,
      travelEtaMinutes: 55,
      departedAt: new Date(T0.getTime() - 15 * 60 * 1000),
      currentBaseSeconds: 24 * 60,
      now: T0,
    });
    expect(ratio).not.toBeNull();
