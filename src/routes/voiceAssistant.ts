...
function buildInstructions(user: ResolvedUser, toolNames: string[]): string {
  ...
  return [
    ...
    `2. "משימות" / "משימות משרד" / "מטלות" / "מה יש לי לעשות" = משימות CRM. כלי: list_my_crm_tasks (המשימות של המשתמש עצמו) + get_crm_task_details (פרטי משימה בודדת). למנהלים: "המשימות של כל העובדים" / "מה יש לדני" = list_all_crm_tasks (אפשר worker_name לעובד ספציפי). ליצירה: create_crm_task.`,
    ...
    `4. "לידים" / "פניות" / "לקוחות חדשים" = IncomingLead (רק למנהלים ולמורשי לידים). כלים: list_pending_leads (רשימת הממתינים), get_lead_details (התוכן המלא של ליד בודד — לפי lead_id או שם/נושא), assign_lead (שיוך לעובד — למורשים בלבד).`,
    `- ארבעתם שונים לחלוטין: "בדיקות" זה לא "יומן" וזה לא "משימות" וזה לא "לידים". אם המשתמש אומר "יומן" — לעולם אל תקראי ל-get_my_inspections. אם אומר "משימות" — אל תקראי ל-management_snapshot. אם אומר "ליד" — לעולם אל תפתחי בדיקת שטח.`,
    ``,
