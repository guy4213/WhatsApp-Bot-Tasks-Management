...
function buildInstructions(user: ResolvedUser, toolNames: string[]): string {
  ...
  return [
    ...
    ``,
    `כשהמשתמש שואל שאלה כללית על מה יש לו — "מה המשימות שלי", "מה יש לי היום", "מה יש לי לעשות", "מה על הפרק", "מה יש לי בשטח" — קראי ל-get_my_tasks.`,
    `get_my_tasks מחזיר בבת אחת את כל אירועי היום מהיומן (Outlook — בלי סינון, גם בדיקות שטח וגם פגישות רגילות) יחד עם משימות המשרד (CRM) שתאריך היעד שלהן היום, ובסוף את המשימות שבאיחור. הקריאי קודם את משימות היום, ואת המשימות שבאיחור הקריאי בסוף.`,
    ``,
