...
function buildInstructions(user: ResolvedUser, toolNames: string[]): string {
  ...
  return [
    ...
    ``,
    `ארבעה עולמות נפרדים — אל תבלבלי ביניהם:`,
    ``,
    `1. בדיקות שטח`,
    `כאשר המשתמש אומר "בדיקות שטח", "משימות שטח", "מה יש לי היום בשטח" או "מה יש לי מחר בשטח", הכוונה לאירועי בדיקות שנוצרו ביומן Outlook על ידי יורם או הצוות.`,
    `הכלי היחיד המתאים הוא get_my_field_tasks.`,
    `אל תקראי ל-get_calendar_events, משום שהוא כולל גם פגישות שאינן בדיקות.`,
    `אל תקראי ל-get_my_inspections, משום שהוא מגיע ממערכת TaskField פנימית שאינה משמשת כיום כמקור בדיקות השטח בפועל.`,
    ``,
    `2. משימות משרד ו-CRM`,
    `כאשר המשתמש אומר "משימות משרד", "משימות רגילות" או "מה יש לי לעשות במשרד", הכוונה למשימות CRM השמורות במסד הנתונים ולא ביומן.`,
    `השתמשי ב-list_my_crm_tasks. לפרטי משימה השתמשי ב-get_crm_task_details. ליצירת משימה — create_crm_task. למנהלים: "המשימות של כל העובדים" / "מה יש לדני" = list_all_crm_tasks (אפשר worker_name לעובד ספציפי).`,
    ``,
    `3. היומן המלא`,
    `כאשר המשתמש אומר "יומן", "מה קבוע לי", "כל הפגישות", "מה יש לי היום ביומן" או "היומן שלי", הכוונה לכל אירועי Outlook ללא סינון.`,
    `השתמשי ב-get_calendar_events. היומן המלא יכול לכלול גם בדיקות שטח וגם פגישות רגילות. ליצירת אירוע — create_calendar_event (לעדכון/מחיקה: update_calendar_event / delete_calendar_event).`,
    ``,
    `4. לידים ופניות חדשות`,
    `כאשר המשתמש אומר "לידים", "פניות" או "לקוחות חדשים", הכוונה ל-IncomingLead. למשתמשים מורשים בלבד השתמשי ב-list_pending_leads, get_lead_details ו-assign_lead.`,
    ``,
    `הבהרה חשובה: Outlook כולל באותו יומן גם בדיקות שטח וגם פגישות רגילות. כשמבקשים "בדיקות שטח" — קראי רק ל-get_my_field_tasks. כשמבקשים "היומן שלי" או "כל הפגישות" — קראי ל-get_calendar_events. אם אומר "ליד" — לעולם אל תפתחי בדיקת שטח.`,
    ``,
